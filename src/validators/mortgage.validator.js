...
function calculateMinimumDownPayment(propertyPrice) {
  ...
  if (propertyPrice < 1500000) {
    return 500000 * 0.05 + (propertyPrice - 500000) * 0.1;
  }
  return propertyPrice * 0.2;
}

function isValid30YearAmortization(
  amortizationYears,
  propertyPrice,
  downPayment,
) {
...
function getValidationErrors(input) {
  ...
  if (downPayment < minDownPayment) {
    const formattedMin = parseFloat(minDownPayment.toFixed(2));
    return `Down payment is below the minimum required amount of $${formattedMin}.`;
  }

  // 30-year Amortization Exception Check
  if (
    !isValid30YearAmortization(amortizationYears, propertyPrice, downPayment)
  ) {
