...
describe("Mortgage Validator", () => {
  describe("calculateMinimumDownPayment", () => {
    ...
    it("should require 5% for the first $500k and 10% for the rest if < $1.5M", () => {
      ...
      expect(calculateMinimumDownPayment(999999)).toBeCloseTo(
        25000 + 49999.9,
        2,
      );
    });
  ...
  describe("getValidationErrors", () => {
    ...
    it("should return error for invalid propertyPrice", () => {
      expect(
        getValidationErrors({ ...validBaseInput, propertyPrice: -100 }),
      ).toBe("propertyPrice must be a number greater than 0.");
      expect(
        getValidationErrors({ ...validBaseInput, propertyPrice: "500000" }),
      ).toBe("propertyPrice must be a number greater than 0.");
    });

    it("should return error for negative downPayment", () => {
      expect(getValidationErrors({ ...validBaseInput, downPayment: -10 })).toBe(
        "downPayment must be a positive number.",
      );
    });

    it("should return error if downPayment >= propertyPrice", () => {
      expect(
        getValidationErrors({ ...validBaseInput, downPayment: 600000 }),
      ).toBe("downPayment must be less than propertyPrice.");
    });

    it("should return error for invalid annualInterestRate (non-positive or >= 100)", () => {
      const errMsg =
        "annualInterestRate must be a positive number between 0 and 100 (e.g., 5 for 5%).";
      expect(
        getValidationErrors({ ...validBaseInput, annualInterestRate: 0 }),
      ).toBe(errMsg);
      expect(
        getValidationErrors({ ...validBaseInput, annualInterestRate: -5 }),
      ).toBe(errMsg);
      expect(
        getValidationErrors({ ...validBaseInput, annualInterestRate: 150 }),
      ).toBe(errMsg);
    });

    it("should return error for invalid amortizationYears", () => {
      expect(
        getValidationErrors({ ...validBaseInput, amortizationYears: 27 }),
      ).toContain("amortizationYears must be one of");
    });

    it("should return error for invalid paymentSchedule", () => {
      expect(
        getValidationErrors({ ...validBaseInput, paymentSchedule: "yearly" }),
      ).toContain("paymentSchedule must be one of");
    });
    ...
    it("should return error if down payment is below minimum required", () => {
      ...
      ).toBe("Down payment is below the minimum required amount of $35000.");
    });
    ...
    it("should return error if 30-year amortization has < 20% down payment", () => {
      ...
      expect(
        ...
        "A 30-year amortization period requires a down payment of at least 20%.",
      );
    });
  });
});
